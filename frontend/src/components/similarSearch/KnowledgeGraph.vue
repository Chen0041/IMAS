...
export default {
  ...
  beforeMount: function() {
    ...
    this.loadKG()
    // TODO ForTest
    console.log(" url: '/kg/statistics/' ")
    this.statistics = {"numOfNodes": 1, "numOfPatients": 1, "numOfAdmissions": 1, "numOfDiseases": 1, "numOfDrugs": 1, "numOfEdges": 1}
  ...
  methods: {
    loadKGList() {
      // TODO ForTest
      console.log(" url: '/kg/user/' + this.$store.state.user.id ")
      this.kgGroupList[1].options.length = 1;
    ...
    searchNode(){
      this.removeCurrentKgData();
      // TODO ForTest
      console.log(" url: '/kg/node/search/' + this.searchContent ")
    ...
    getRelNodes(nodeId) {
      if (this.kgNodeBufferedMap[nodeId].childNode.length === 0) {
        // TODO ForTest
        console.log(" url: '/kg/graph/relNodes/' + nodeId ")
    ...
    initGraph() {
      ...
      // TODO ERROR TypeError: Cannot read properties of undefined (reading 'offsetWidth'
      let width = 1260;
      let height = 480;
